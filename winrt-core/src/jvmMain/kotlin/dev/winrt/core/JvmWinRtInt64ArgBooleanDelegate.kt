...
class JvmWinRtInt64ArgBooleanDelegate private constructor(
    ...
    companion object {
        ...
        private val queryInterfaceStub = linker.upcallStub(
            lookup.findStatic(
                JvmWinRtInt64ArgBooleanDelegate::class.java,
                "queryInterface",
        ...
        private val invokeStub = linker.upcallStub(
            lookup.findStatic(
                JvmWinRtInt64ArgBooleanDelegate::class.java,
                "invoke",
                MethodType.methodType(Int::class.javaPrimitiveType, MemorySegment::class.java, Long::class.javaPrimitiveType, MemorySegment::class.java),
            ),
            FunctionDescriptor.of(ValueLayout.JAVA_INT, ValueLayout.ADDRESS, ValueLayout.JAVA_LONG, ValueLayout.ADDRESS),
            libraryArena,
        )

        fun create(iid: Guid, invoke: (Long) -> Boolean): JvmWinRtInt64ArgBooleanDelegate {
            ...
            return JvmWinRtInt64ArgBooleanDelegate(arena, instance)
        }
        ...
        @JvmStatic
        private fun invoke(thisPointer: MemorySegment, arg: Long, result: MemorySegment): Int {
            val state = states[thisPointer.address()] ?: return KnownHResults.E_POINTER.value
            result.reinterpret(ValueLayout.JAVA_INT.byteSize().toLong()).set(
                ValueLayout.JAVA_INT,
                0,
                if (state.invoke(arg)) 1 else 0,
            )
