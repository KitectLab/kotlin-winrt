...
class JvmWinRtStringArgBooleanDelegate private constructor(
    ...
    companion object {
        ...
        private val queryInterfaceStub = linker.upcallStub(
            lookup.findStatic(
                JvmWinRtStringArgBooleanDelegate::class.java,
                "queryInterface",
        ...
        fun create(iid: Guid, invoke: (String) -> Boolean): JvmWinRtStringArgBooleanDelegate {
            ...
            return JvmWinRtStringArgBooleanDelegate(arena, instance)
        }
        ...
        private fun invoke(thisPointer: MemorySegment, arg: MemorySegment, result: MemorySegment): Int {
            val state = states[thisPointer.address()] ?: return KnownHResults.E_POINTER.value
            val hString = HString(arg.address())
            result.reinterpret(ValueLayout.JAVA_INT.byteSize().toLong()).set(
                ValueLayout.JAVA_INT,
                0,
                if (state.invoke(PlatformHStringBridge.toKotlinString(hString))) 1 else 0,
            )
