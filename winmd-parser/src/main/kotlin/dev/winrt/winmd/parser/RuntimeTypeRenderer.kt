...
internal class RuntimeTypeRenderer(
    ...
    private fun supportsFactoryConstructor(candidate: RuntimeClassFactoryMethod): Boolean {
        ...
        return !constructorParameters.requiresArrayMarshaling() ||
            (
                candidate.method.copy(parameters = constructorParameters).isInt32PassArrayMethod { _ -> true } &&
            ...
            (
                candidate.method.copy(parameters = constructorParameters).isUInt32PassArrayMethod { _ -> true } &&
                    constructorParameters
                        .filterNot { parameter -> parameter.isUInt32PassArrayParameter() }
                        .all { parameter ->
                            methodParameterCategory(
                                if (typeRegistry.isEnumType(parameter.type, candidate.runtimeClass.namespace)) {
                                    enumSignatureType(typeRegistry, parameter.type, candidate.runtimeClass.namespace)
            ...
            (
                candidate.method.copy(parameters = constructorParameters).isUInt8PassArrayMethod { _ -> true } &&
                    constructorParameters
                        .filterNot { parameter -> parameter.isUInt8PassArrayParameter() }
                        .all { parameter ->
                            methodParameterCategory(
                                if (typeRegistry.isEnumType(parameter.type, candidate.runtimeClass.namespace)) {
                                    enumSignatureType(typeRegistry, parameter.type, candidate.runtimeClass.namespace)
            ...
            (
                candidate.method.copy(parameters = constructorParameters).isInt16PassArrayMethod { _ -> true } &&
                    constructorParameters
                        .filterNot { parameter -> parameter.isInt16PassArrayParameter() }
                        .all { parameter ->
                            methodParameterCategory(
                                if (typeRegistry.isEnumType(parameter.type, candidate.runtimeClass.namespace)) {
                                    enumSignatureType(typeRegistry, parameter.type, candidate.runtimeClass.namespace)
            ...
            (
                candidate.method.copy(parameters = constructorParameters).isUInt16PassArrayMethod { _ -> true } &&
                    constructorParameters
                        .filterNot { parameter -> parameter.isUInt16PassArrayParameter() }
                        .all { parameter ->
                            methodParameterCategory(
                                if (typeRegistry.isEnumType(parameter.type, candidate.runtimeClass.namespace)) {
                                    enumSignatureType(typeRegistry, parameter.type, candidate.runtimeClass.namespace)
            ...
            (
                candidate.method.copy(parameters = constructorParameters).isChar16PassArrayMethod { _ -> true } &&
                    constructorParameters
                        .filterNot { parameter -> parameter.isChar16PassArrayParameter() }
                        .all { parameter ->
                            methodParameterCategory(
                                if (typeRegistry.isEnumType(parameter.type, candidate.runtimeClass.namespace)) {
                                    enumSignatureType(typeRegistry, parameter.type, candidate.runtimeClass.namespace)
            ...
            (
                candidate.method.copy(parameters = constructorParameters).isFloat32PassArrayMethod { _ -> true } &&
                    constructorParameters
                        .filterNot { parameter -> parameter.isFloat32PassArrayParameter() }
                        .all { parameter ->
                            methodParameterCategory(
                                if (typeRegistry.isEnumType(parameter.type, candidate.runtimeClass.namespace)) {
                                    enumSignatureType(typeRegistry, parameter.type, candidate.runtimeClass.namespace)
            ...
            (
                candidate.method.copy(parameters = constructorParameters).isFloat64PassArrayMethod { _ -> true } &&
                    constructorParameters
                        .filterNot { parameter -> parameter.isFloat64PassArrayParameter() }
                        .all { parameter ->
                            methodParameterCategory(
                                if (typeRegistry.isEnumType(parameter.type, candidate.runtimeClass.namespace)) {
                                    enumSignatureType(typeRegistry, parameter.type, candidate.runtimeClass.namespace)
    ...
    private fun runtimeInterfaceTypes(type: WinMdType): List<WinMdType> {
        ...
        val defaultInterfaceName = defaultInterfaceType?.let { canonicalWinRtInterfaceName("${it.namespace}.${it.name}") }
        ...
        val collectionBackedInterfaces = buildSet {
            ...
            collectionSuperinterface(interfaceName, type.namespace, emptySet()) != null ||
                isWinRtCollectionInterface(interfaceName)
        }
        return buildList {
            ...
            (type.implementedInterfaces + type.baseInterfaces)
                .filter {
                    typeRegistry.isRuntimeProjectedInterface(it, type.namespace) &&
                        ...
                        canonicalWinRtInterfaceName(it) !in inheritedByDefault &&
    ...
    private fun allImplementedInterfaceNames(type: WinMdType): Set<String> {
        return buildSet {
            add(canonicalWinRtInterfaceName("${type.namespace}.${type.name}"))
            type.baseInterfaces.forEach { baseInterface ->
                add(canonicalWinRtInterfaceName(baseInterface))
