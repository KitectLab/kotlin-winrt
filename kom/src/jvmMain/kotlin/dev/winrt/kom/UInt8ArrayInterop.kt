...
private val coTaskMemFreeUInt8Handle by lazy {
...
private fun uint8ReceiveArrayMethodHandle(argumentLayouts: List<MemoryLayout>) = Jdk22Foreign.downcallHandle(
...
actual fun invokeUInt8ReceiveArrayMethod(instance: ComPtr, vtableIndex: Int, vararg arguments: Any): Result<ByteArray> {
    return runCatching {
        ...
        Arena.ofConfined().use { arena ->
            ...
            try {
                val hresult = HResult(
                    uint8ReceiveArrayMethodHandle(arguments.map(::methodArgumentLayout)).bindTo(function).invokeWithArguments(
                ...
                hresult.requireSuccess("invokeUInt8ReceiveArrayMethod($vtableIndex)")

                val size = sizeSegment.get(ValueLayout.JAVA_INT, 0L)
                check(size >= 0) { "Negative UInt8 array size returned from slot $vtableIndex: $size" }
                ...
                check(dataPointer.address() != 0L) {
                    "Null UInt8 array buffer returned from slot $vtableIndex with size $size"
                }
                ...
                if (dataPointer.address() != 0L) {
                    coTaskMemFreeUInt8Handle.invokeWithArguments(dataPointer)
                }
            }
        }
    }
