...
private val coTaskMemFreeFloat64Handle by lazy {
...
private fun float64ReceiveArrayMethodHandle(argumentLayouts: List<MemoryLayout>) = Jdk22Foreign.downcallHandle(
...
actual fun invokeFloat64ReceiveArrayMethod(instance: ComPtr, vtableIndex: Int, vararg arguments: Any): Result<DoubleArray> {
    return runCatching {
        ...
        Arena.ofConfined().use { arena ->
            ...
            try {
                val hresult = HResult(
                    float64ReceiveArrayMethodHandle(arguments.map(::methodArgumentLayout)).bindTo(function).invokeWithArguments(
                ...
                hresult.requireSuccess("invokeFloat64ReceiveArrayMethod($vtableIndex)")

                val size = sizeSegment.get(ValueLayout.JAVA_INT, 0L)
                check(size >= 0) { "Negative Float64 array size returned from slot $vtableIndex: $size" }

                dataPointer = dataSegment.get(ValueLayout.ADDRESS, 0L)
                if (size == 0) {
                    return@use DoubleArray(0)
                }
                check(dataPointer.address() != 0L) {
                    "Null Float64 array buffer returned from slot $vtableIndex with size $size"
                }
                ...
                if (dataPointer.address() != 0L) {
                    coTaskMemFreeFloat64Handle.invokeWithArguments(dataPointer)
                }
            }
        }
    }
