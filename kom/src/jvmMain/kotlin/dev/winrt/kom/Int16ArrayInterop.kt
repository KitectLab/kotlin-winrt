...
private val coTaskMemFreeInt16Handle by lazy {
...
private fun int16ReceiveArrayMethodHandle(argumentLayouts: List<MemoryLayout>) = Jdk22Foreign.downcallHandle(
...
actual fun invokeInt16ReceiveArrayMethod(instance: ComPtr, vtableIndex: Int, vararg arguments: Any): Result<ShortArray> {
    return runCatching {
        ...
        Arena.ofConfined().use { arena ->
            ...
            try {
                val hresult = HResult(
                    int16ReceiveArrayMethodHandle(arguments.map(::methodArgumentLayout)).bindTo(function).invokeWithArguments(
                ...
                hresult.requireSuccess("invokeInt16ReceiveArrayMethod($vtableIndex)")

                val size = sizeSegment.get(ValueLayout.JAVA_INT, 0L)
                check(size >= 0) { "Negative Int16 array size returned from slot $vtableIndex: $size" }
                ...
                check(dataPointer.address() != 0L) {
                    "Null Int16 array buffer returned from slot $vtableIndex with size $size"
                }
                ...
                if (dataPointer.address() != 0L) {
                    coTaskMemFreeInt16Handle.invokeWithArguments(dataPointer)
                }
            }
        }
    }
