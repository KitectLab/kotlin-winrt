            ...
            try {
                ...
                try {
                    ...
                    try {
                        ...
                        assertTrue(projected.lastEra.value >= projected.firstEra.value)
                        assertTrue(projected.numberOfEras.value > 0)
                        projected.era = projected.era
                        projected.addEras(Int32(0))
                        assertFalse(projected.eraAsString().isBlank())
                        assertFalse(projected.eraAsString(Int32(3)).isBlank())
                        assertTrue(projected.firstYearInThisEra.value > 0)
                        assertTrue(projected.lastYearInThisEra.value >= projected.firstYearInThisEra.value)
                        assertTrue(projected.numberOfYearsInThisEra.value > 0)
                        assertTrue(projected.dateTime.universalTime > 0)
                        projected.setDateTime(projected.dateTime)
                        projected.setToNow()
                        assertTrue(projected.firstMonthInThisYear.value > 0)
                        assertTrue(projected.lastMonthInThisYear.value >= projected.firstMonthInThisYear.value)
                        assertTrue(projected.numberOfMonthsInThisYear.value > 0)
                        assertTrue(projected.month.value > 0)
                        projected.addMonths(Int32(0))
                        projected.addWeeks(Int32(0))
                        assertTrue(projected.firstDayInThisMonth.value > 0)
                        assertTrue(projected.lastDayInThisMonth.value >= projected.firstDayInThisMonth.value)
                        assertTrue(projected.numberOfDaysInThisMonth.value > 0)
                        assertTrue(projected.day.value > 0)
                        projected.addDays(Int32(0))
                        assertTrue(projected.firstPeriodInThisDay.value >= 0)
                        assertTrue(projected.lastPeriodInThisDay.value >= projected.firstPeriodInThisDay.value)
                        assertTrue(projected.numberOfPeriodsInThisDay.value > 0)
                        projected.period = projected.period
                        projected.addPeriods(Int32(0))
                        assertTrue(projected.hour.value >= 0)
                        assertTrue(projected.firstHourInThisPeriod.value >= 0)
                        assertTrue(projected.lastHourInThisPeriod.value >= projected.firstHourInThisPeriod.value)
                        assertTrue(projected.numberOfHoursInThisPeriod.value > 0)
                        projected.hour = projected.hour
                        projected.addHours(Int32(0))
                        assertTrue(projected.minute.value >= 0)
                        assertTrue(projected.firstMinuteInThisHour.value >= 0)
                        assertTrue(projected.lastMinuteInThisHour.value >= projected.firstMinuteInThisHour.value)
                        assertTrue(projected.numberOfMinutesInThisHour.value > 0)
                        projected.minute = projected.minute
                        projected.addMinutes(Int32(0))
                        assertTrue(projected.second.value >= 0)
                        assertTrue(projected.firstSecondInThisMinute.value >= 0)
                        assertTrue(projected.lastSecondInThisMinute.value >= projected.firstSecondInThisMinute.value)
                        assertTrue(projected.numberOfSecondsInThisMinute.value > 0)
                        ...
                        assertFalse(projected.dayOfWeekAsString(Int32(3)).isBlank())
                        assertFalse(projected.dayOfWeekAsSoloString().isBlank())
                        assertFalse(projected.dayOfWeekAsSoloString(Int32(3)).isBlank())
                        projected.periodAsString()
                        projected.periodAsString(Int32(2))
